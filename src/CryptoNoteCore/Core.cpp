...
bool core::getBlockReward(uint32_t height,size_t medianSize, size_t currentBlockSize, uint64_t alreadyGeneratedCoins, uint64_t fee,
                          uint64_t& reward, int64_t& emissionChange) {
  return m_currency.getBlockReward(height,medianSize, currentBlockSize, alreadyGeneratedCoins, fee, reward, emissionChange);
}
